...
def main():
    ...
    for d in data:
        try:
            sim_one = similarity(d[0], d[1], False)
            sim_two = similarity(d[0], d[1], True)
            print "%s\t%s\t%d\t%.3f\t%.3f" % (d[0], d[1], d[2], sim_one, sim_two)
            data_prediction.append([d[0], d[1], d[2], sim_one, sim_two])
           
        except:
            print "Except\t %s\t%s\t%d" % (d[0], d[1], d[2])
            data_prediction.append([d[0], d[1], d[2], 0, 0])

    (predictions, percent) = predict_with_average(data_prediction)
    print predictions
    print percent
